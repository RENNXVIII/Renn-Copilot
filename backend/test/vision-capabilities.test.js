...
test("custom-provider models can be routed through the compatibility proxy", () => {
    const entry = toCopilotModelEntry(
        {
            id: "gpt-sol",
            label: "GPT Sol",
        ...
        {
            proxyUrl: "http://127.0.0.1:8317",
            ownBaseUrl: "http://127.0.0.1:4317",
            forceCompatibilityProxy: true,
        }
    );
    assert.equal(entry.url, "http://127.0.0.1:4317/api/proxy/v1/chat/completions");
});