...
test("Cowork mode applies only to Claude-provider Claude models", () => {
    assert.equal(shouldUseClaudeCoworkMode("claude", "claude-sonnet-4-6", true), true);
    assert.equal(shouldUseClaudeCoworkMode("antigravity", "claude-sonnet-4-6", true), false);
    assert.equal(shouldUseClaudeCoworkMode("my-custom-provider", "claude-sonnet-4-6", true), false);
    assert.equal(shouldUseClaudeCoworkMode("claude", "gemini-3-pro", true), false);
    assert.equal(shouldUseClaudeCoworkMode("claude", "claude-sonnet-4-6", false), false);
});
...
test("findRejectedOptionalParameter detects nested upstream temperature errors", () => {
    ...
    assert.equal(findRejectedOptionalParameter(message), "temperature");
    assert.equal(findRejectedOptionalParameter("ordinary bad request"), null);
});

test("applyRequestCompatibility removes only scoped optional parameters", () => {
    const body = { model: "gpt-sol", temperature: 0.1, top_p: 0.8, messages: [] };
    assert.equal(applyRequestCompatibility(body, "standard"), body);

    const auto = applyRequestCompatibility(body, "auto", ["temperature"]);
    assert.equal(auto.temperature, undefined);
    assert.equal(auto.top_p, 0.8);
    assert.deepEqual(auto.messages, []);
    assert.equal(body.temperature, 0.1, "input body must not be mutated");

    const strict = applyRequestCompatibility(body, "strict");
    assert.equal(strict.temperature, undefined);
    assert.equal(strict.top_p, undefined);
    assert.deepEqual(strict.messages, []);
});

test("requestCompatibilityKey scopes learned rules by provider and model", () => {
    assert.equal(requestCompatibilityKey("ohhmyagent", "gpt-sol"), "ohhmyagent::gpt-sol");
});

test("proxy Auto mode persists a learned rejected parameter and retries", async (t) => {
    const previousHome = settings.cliproxyHome;
    const temporaryHome = fs.mkdtempSync(path.join(os.tmpdir(), "renn-compat-test-"));
    settings.cliproxyHome = temporaryHome;
    t.after(() => {
        settings.cliproxyHome = previousHome;
    ...
    globalThis.fetch = async (_url, init) => {
        ...
        if (requests.length === 1) {
            return new Response(
                JSON.stringify({ error: { message: "Unsupported value: 'temperature'. Only the default (1) value is supported." } }),
    ...
    const { proxyChatCompletions } = await import("../src/chat-proxy.js");
    const chunks = [];
    ...
    res.send = function send(value) { this.end(value); return this; };
    ...
    await proxyChatCompletions(
        { body: { model: "gpt-sol", temperature: 0.1, stream: false, messages: [] }, headers: {} },
        res
    );
    await finished;
