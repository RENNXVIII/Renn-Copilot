...
/** Cowork mode belongs exclusively to the Claude / Claude Code provider. */
export function shouldUseClaudeCoworkMode(provider, model, enabled) {
    return Boolean(enabled && provider === "claude" && isClaudeModelId(model));
}

const OPTIONAL_REQUEST_PARAMETERS = Object.freeze([
    "temperature",
    "top_p",
    "frequency_penalty",
    "presence_penalty",
...
export function requestCompatibilityKey(provider, model) {
    return `${provider || "other"}::${model || "unknown"}`;
}

export function findRejectedOptionalParameter(message) {
    if (typeof message !== "string") return null;
    const lower = message.toLowerCase();
    return OPTIONAL_REQUEST_PARAMETERS.find((parameter) => {
        const index = lower.indexOf(parameter);
        if (index === -1) return false;
        const context = lower.slice(Math.max(0, index - 120), index + parameter.length + 160);
        return /unsupported|not supported|does not support|only the default/.test(context);
    }) || null;
}

export function applyRequestCompatibility(body, mode, learnedParameters = []) {
    if (!body || typeof body !== "object" || mode === "standard") return body;
    const parameters = mode === "strict" ? OPTIONAL_REQUEST_PARAMETERS : learnedParameters;
    if (!parameters.some((parameter) => Object.prototype.hasOwnProperty.call(body, parameter))) return body;
    const next = { ...body };
    for (const parameter of parameters) delete next[parameter];
    return next;
}
...
export async function proxyChatCompletions(req, res) {
    ...
    const claudeCoworkMode = Boolean(state.claudeCoworkMode);
    const provider = state.modelProviderMemory?.[body.model] || "other";
    const cloak = shouldUseClaudeCoworkMode(provider, body.model, claudeCoworkMode);
    const compatibilityMode = state.customProviderCompatibility?.[provider] || "auto";
    const compatibilityKey = requestCompatibilityKey(provider, body.model);
    const learnedParameters = state.modelRequestCompatibility?.[compatibilityKey]?.remove || [];

    let forwardBody = applyRequestCompatibility(body, compatibilityMode, learnedParameters);
    let reverseMap = null;

    if (cloak) {
        const cloaked = applyCoworkToolCloak(forwardBody);
    ...
    const fetchUpstream = (requestBody) => fetch(`${proxyBaseUrl()}/v1/chat/completions`, {
        method: "POST",
    ...
    let upstream = await fetchUpstream(forwardBody);

    // Auto mode retries one rejected request after removing only the optional
    // parameter named by the upstream. A 400 means generation did not start,
    // so this does not duplicate a successful completion. The learned rule is
    // scoped to provider + model and applied before future requests.
    if (compatibilityMode === "auto" && upstream.status === 400) {
        const errorText = await upstream.text();
        const rejectedParameter = findRejectedOptionalParameter(errorText);
        if (rejectedParameter && Object.prototype.hasOwnProperty.call(forwardBody, rejectedParameter)) {
            const retryBody = { ...forwardBody };
            delete retryBody[rejectedParameter];
            const latestState = readState();
            const compatibility = { ...(latestState.modelRequestCompatibility || {}) };
            const previous = compatibility[compatibilityKey]?.remove || [];
            compatibility[compatibilityKey] = {
                remove: Array.from(new Set([...previous, rejectedParameter])),
                learnedAt: Date.now(),
            };
            writeState({ modelRequestCompatibility: compatibility });
            console.log(`[compat] ${compatibilityKey}: removed rejected ${rejectedParameter} and retried once`);
            forwardBody = retryBody;
            upstream = await fetchUpstream(forwardBody);
        } else {
