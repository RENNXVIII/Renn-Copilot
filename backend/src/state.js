...
const defaultState = {
    ...
    // Request compatibility policy for custom OpenAI-compatible providers.
    // Values are "auto" (retry once and learn rejected parameters), "standard"
    // (pass through unchanged), or "strict" (remove optional sampling knobs).
    customProviderCompatibility: {},
    // Parameters learned from upstream 400 responses, keyed by
    // "<provider>::<model-id>". Kept outside CLIProxyAPI config because its
    // Management API only accepts fields it knows about.
