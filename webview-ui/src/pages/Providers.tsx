...
import { api, type ApiKeyEntry, type AuthFileEntry, type OpenAiCompatEntry, type RequestCompatibilityMode } from "../api/client";
...
export function Providers() {
    ...
    const [apiKeyModal, setApiKeyModal] = useState<{ config: NonNullable<(typeof PROVIDER_CARDS)[number]["apiKey"]>; editIndex?: number } | null>(null);
    const [xaiModal, setXaiModal] = useState<{ editIndex?: number } | null>(null);
    const [customModal, setCustomModal] = useState<{ editIndex?: number } | null>(null);
                        ...
                        {keyEntries.map((k, i) => (
                            <div key={`xai-key-${i}`} className="cred-row">
                                ...
                                <div className="btn-row">
                                    <button className="btn secondary" onClick={() => setXaiModal({ editIndex: i })}>Edit</button>
                                    <button className="btn secondary" onClick={() => removeXaiKey(i)}>Remove</button>
                                </div>
                            </div>
                        ))}
                    </>
                ...
                <>
                    ...
                    {keyItems.map((entry, i) => (
                        <div key={`key-${i}`} className="cred-row">
                            ...
                            <button
                                className="btn secondary"
                                onClick={() => setApiKeyModal({ config: keyConfig, editIndex: i })}
                            >
                                Edit
                            </button>
                            <button
            ...
            <>
                ...
                {groupItems.map(({ entry, idx }) => (
                    <div key={entry.name} className="cred-row">
                        ...
                        <div className="btn-row">
                            <button className="btn secondary" onClick={() => setCustomModal({ editIndex: idx })}>
                                Edit
                            </button>
                            <button
                                className="btn secondary"
        ...
        <div className="page">
            ...
            <div className="grid">
                        ...
                        <div className="card" key={p.id}>
                            ...
                            <div className="btn-row" style={{ flexDirection: "column" }}>
                                ...
                                <button
                                    className="btn secondary"
                                    style={{ width: "100%" }}
                                    onClick={() => (p.isXai ? setXaiModal({}) : setApiKeyModal({ config: p.apiKey! }))}
                ...
                <div className="card">
                    ...
                    <button className="btn secondary" style={{ width: "100%" }} onClick={() => setCustomModal({})} disabled={!serverRunning}>
                        Add custom provider
                    </button>
                </div>
            </div>
            ...
            <Modal open={apiKeyModal !== null} onClose={() => setApiKeyModal(null)}>
                {apiKeyModal && <ApiKeyModalContent label={apiKeyModal.config.label} getter={apiKeyModal.config.getter} setter={apiKeyModal.config.setter} editIndex={apiKeyModal.editIndex} onSaved={() => {
                    geminiKeysQuery.mutate(undefined, true);
                    claudeKeysQuery.mutate(undefined, true);
                    codexKeysQuery.mutate(undefined, true);
                }} onClose={() => setApiKeyModal(null)} />}
            </Modal>

            <Modal open={xaiModal !== null} onClose={() => setXaiModal(null)}>
                {xaiModal && <XaiApiKeyModalContent editIndex={xaiModal.editIndex} onSaved={() => xaiKeyQuery.mutate(undefined, true)} onClose={() => setXaiModal(null)} />}
            </Modal>

            <Modal open={customModal !== null} onClose={() => setCustomModal(null)}>
                {customModal && <CustomProviderModalContent editIndex={customModal.editIndex} onSaved={() => customQuery.mutate(undefined, true)} onClose={() => setCustomModal(null)} groupOptions={groupOptions} onAssignGroup={assignCustomGroup} />}
            </Modal>
...
function ApiKeyModalContent({
    ...
    useEffect(() => {
        const entry = editIndex === undefined ? undefined : items[editIndex];
        setApiKey(entry?.["api-key"] ?? "");
        setBaseUrl(entry?.["base-url"] ?? "");
    }, [editIndex, data]);
    ...
    async function saveEntry() {
        ...
        setSaving(true);
        try {
            await setter(editIndex === undefined ? [...items, entry] : items.map((item, i) => (i === editIndex ? { ...item, ...entry, "base-url": entry["base-url"] } : item)));
            setApiKey("");
            setBaseUrl("");
        } finally {
            setSaving(false);
    ...
    return (
        <>
            <ModalHeader title={`${editIndex === undefined ? "Add" : "Edit"} ${label}`} description={`${items.length} key${items.length === 1 ? "" : "s"} configured`} onClose={onClose} />
            ...
            <div className="field" style={{ border: "1px dashed var(--vscode-panel-border)", borderRadius: 4, padding: 10, gap: 8 }}>
                ...
                <button className="btn" style={{ alignSelf: "flex-start" }} disabled={saving || !apiKey.trim()} onClick={saveEntry}>
                    {editIndex === undefined ? "Add" : "Save changes"}
                </button>
            </div>
        </>
    );
...
function XaiApiKeyModalContent({ editIndex, onSaved, onClose }: { editIndex?: number; onSaved: () => void; onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        setApiKey(editIndex === undefined ? "" : keyEntries[editIndex]?.["api-key"] ?? "");
    }, [entry, editIndex]);
    ...
    return (
        <>
            <ModalHeader title={`${editIndex === undefined ? "Add" : "Edit"} xAI API Key`} description={`${keyEntries.length} key${keyEntries.length === 1 ? "" : "s"} configured`} onClose={onClose} />
            {isLoading && <p className="card-desc">Loading...</p>}
            ...
            <div className="field" style={{ border: "1px dashed var(--vscode-panel-border)", borderRadius: 4, padding: 10, gap: 8 }}>
                ...
                <div className="btn-row">
                    <button className="btn" disabled={saving || !apiKey.trim()} onClick={addEntry}>
                        {editIndex === undefined ? "Add key" : "Save changes"}
                    </button>
                    {keyEntries.length > 0 && (
...
function CustomProviderModalContent({
    ...
    const [compatibilityMode, setCompatibilityMode] = useState<RequestCompatibilityMode>("auto");
    const [saving, setSaving] = useState(false);
    const [saveError, setSaveError] = useState<string | null>(null);
    const compatibilityQuery = usePolling(api.getRequestCompatibility, 30000);
    const formInitialized = useRef(false);
    const compatibilityInitialized = useRef(editIndex === undefined);

    useEffect(() => {
        if (formInitialized.current || !data) return;
        const entry = editIndex === undefined ? undefined : items[editIndex];
        setName(entry?.name ?? "");
        setBaseUrl(entry?.["base-url"] ?? "");
        setApiKey(entry?.["api-key-entries"]?.[0]?.["api-key"] ?? "");
        setGroup(entry ? loadCustomGroups()[entry.name] || "Ungrouped" : "");
        setModelIds((entry?.models ?? []).map((m) => m.alias || m.name).join(", "));
        formInitialized.current = true;
    }, [editIndex, data, items]);

    useEffect(() => {
        if (compatibilityInitialized.current || !compatibilityQuery.data || !data) return;
        const entry = editIndex === undefined ? undefined : items[editIndex];
        setCompatibilityMode(entry ? compatibilityQuery.data.providers?.[entry.name] || "auto" : "auto");
        compatibilityInitialized.current = true;
    }, [editIndex, data, items, compatibilityQuery.data]);
    ...
    async function saveEntry() {
        ...
        setSaveError(null);
        try {
            const previousName = editIndex === undefined ? null : items[editIndex]?.name;
            await api.setOpenAiCompat(editIndex === undefined ? [...items, entry] : items.map((item, i) => i === editIndex ? { ...item, ...entry } : item));
            await api.setRequestCompatibility(entry.name, compatibilityMode, previousName || undefined);
            onAssignGroup(entry.name, group.trim());
            if (previousName && previousName !== entry.name) {
                const groups = loadCustomGroups();
                delete groups[previousName];
    ...
    return (
        <>
            <ModalHeader title={`${editIndex === undefined ? "Add" : "Edit"} custom provider`} description="GLM, Kimi/Moonshot, or any other endpoint that speaks the OpenAI chat-completions API." onClose={onClose} />
            {isLoading && <p className="card-desc">Loading...</p>}
            {!isLoading && <div className="field" style={{ border: "1px dashed var(--vscode-panel-border)", borderRadius: 4, padding: 10, gap: 8 }}>
                <div className="field">
                ...
                <div className="field">
                    <label className="field-label">Request compatibility</label>
                    <select
                        className="text-input"
                        value={compatibilityMode}
                        onChange={(e) => setCompatibilityMode(e.target.value as RequestCompatibilityMode)}
                    >
                        <option value="auto">Auto (recommended)</option>
                        <option value="standard">Standard OpenAI</option>
                        <option value="strict">Strict defaults</option>
                    </select>
                    <p className="card-desc">
                        Auto retries once when a model rejects an optional parameter such as temperature, then remembers the fix for that provider and model. Strict removes optional sampling parameters immediately. Other providers are unaffected.
                    </p>
                </div>
                {saveError && <p className="card-desc" style={{ color: "var(--vscode-errorForeground)" }} role="alert">{saveError}</p>}
                <button className="btn" style={{ alignSelf: "flex-start" }} disabled={saving || !name.trim() || !baseUrl.trim() || !apiKey.trim() || !group.trim()} onClick={saveEntry}>
                    {saving ? "Saving..." : editIndex === undefined ? "Add provider" : "Save changes"}
                </button>
            </div>}
        </>
    );
